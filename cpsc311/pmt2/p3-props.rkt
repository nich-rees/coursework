...
(define (interp-prop p env)
  (type-case Prop p
    ...
    [not/P (p) (let/eff ([x (interp-prop p env)])
                        (return/eff (not x)))]
    [and/P (p1 p2) (let/eff* ([lhs (interp-prop p1 env)]
                              [rhs (interp-prop p2 env)])
                             (return/eff (and lhs rhs)))]
    [or/P (p1 p2) (let/eff* ([lhs (interp-prop p1 env)]
                             [rhs (interp-prop p2 env)])
                            (return/eff (or lhs rhs)))]
    [implies/P (p1 p2) (let/eff* ([lhs (interp-prop p1 env)]
                                  [rhs (interp-prop p2 env)])
                                 (return/eff (or (not lhs) rhs)))]))
