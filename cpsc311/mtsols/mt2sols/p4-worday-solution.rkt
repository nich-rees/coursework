...
(define (slctns str)
  (local [;; (listof Character) -> SlctnComputation
          (define (slctn/eff chars)
            (cond [(empty? chars) (return/eff "")]
                  [else
                   (let/eff ([result (slctn/eff (rest chars))])
                            (amb/eff
                             (return/eff (string-append (string (first chars))
                                                        result))
                             (return/eff result)))]))]
    (run/eff (slctn/eff (string->list str)) #f)))
...
(define (perms str)
  (local [;; (listof Character) -> PermComputation
          (define (perm/eff chars)
            (cond [(empty? chars) (return/eff empty)]
                  [else ;; cons?
                   (let/eff ([result (perm/eff (rest chars))])
                            (insert/eff (first chars) result))]))

          ;; Character (listof Character) -> PermsComputation
          (define (insert/eff char loc)
            (cond [(empty? loc) (return/eff (list char))]
                  [else ;; cons?
                   (amb/eff (return/eff (cons char loc))
                            (let/eff ([result (insert/eff char (rest loc))])
                                     (return/eff
                                      (cons (first loc) result))))]))]
    (let ([char-lists (run/eff (perm/eff (string->list str)) #f)])
      (map list->string char-lists))))

(test (perms "") (list ""))
(test (perms "a") (list "a"))
(test (perms "ap") (list "ap" "pa"))
(test (perms "ape") (list "ape" "pae" "pea" "aep" "eap" "epa"))

;; An alternate solution
#;
(define (perms str)
  (local
    [;; (listof Character) -> PermComputation
     ;; produce some permutation of chars
     (define (perm/eff chars)
       (cond [(empty? chars) (return/eff '())]
             [else (perm-nonempty/eff chars)]))

     ;; (nelistof Character) -> PermComputation
     (define (perm-nonempty/eff chars0)
       (local [;; Character -> PermComputation
               ;; produce a permutation of chars0 headed by h
               ;; INVARIANT: h is an element of chars0
               (define (perm-headed-by h)
                 (let/eff ([perm-rest (perm/eff (remove h chars0))])
                          (return/eff (cons h perm-rest))))

               ;; (nelistof Character) -> PermComputation
               ;; produce a permutation of chars0 headed by some char in chars
               ;; INVARIANT: chars is a suffix of chars0
               (define (recur chars)
                 (cond [(empty? (rest chars)) (perm-headed-by (first chars))]
                       [else (amb/eff (perm-headed-by (first chars))
                                      (recur (rest chars)))]))]
         (recur chars0)))]
    (let ([char-lists (run/eff (perm/eff (string->list str)) #f)])
      (map list->string char-lists))))
