...
(define (interp/ittl tl0)
  (local [(define (fn-for-tl tl env)
             (type-case TL tl
               [num (n) n]
               [add (lhs rhs) (+ (fn-for-tl lhs env) (fn-for-tl rhs env))]
               [id (x) (lookup-env x)]
               [fixFun (f x body) (fixFunV f x (fn-for-tl body) env)]
               [app (rator rand) (let ([result (fn-for-tl rand env)])
                                   (fn-for-tl rator) )]
               [tuple (args) (... args)]
               [match-with (ids named body)
                           (... ids (fn-for-tl named) (fn-for-tl body))]))]
    (fn-for-tl tl0 empty-env)))
