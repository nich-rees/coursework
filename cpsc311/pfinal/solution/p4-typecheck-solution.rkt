...
(define (check-match-with/wt ids types named/wt body/wt)
  (and
   (= (length ids) (length types))
   (unique? ids)
   (equal? (tupleType types) (tl/wt-type named/wt))
   (equal? (tl/wt-tenv body/wt) (extend-env (tl/wt-tenv named/wt) ids types))))
...
;; SOLUTION EXAMPLES  **NEED MATCH-WITH EXAMPLES!!!***
(test/data (tuple/wt empty
                     (list
                      (num/wt empty 6)
                      (num/wt empty 7)
                      (tuple/wt empty empty))))
     ...
     (define (tl->tl/wtX--tenvX tl tenvX)
       (type-case TL tl
         ...
         [tuple (args)
                ...
                (tuple/wtX tenvX
                           (map (λ (arg)
                                  (tl->tl/wtX--tenvX arg tenvX))
                                args))
                ]
         [match-with (ids named body)
                     #;(... ids named body)
                     (let* ([typeXs (map (λ (id) (some-type!)) ids)]
                            [named/wtX (synth&assert named
                                                     tenvX
                                                     (tupleType typeXs))]
                            [body/wtX
                             (tl->tl/wtX--tenvX
                              body
                              (extend-env tenvX ids typeXs))])
