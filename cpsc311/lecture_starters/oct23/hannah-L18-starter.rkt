...
(define (apply-value v1 v2 store)
  (type-case Value v1
    [funV (x body env) (interp-hg-acc body (extend-env env x v2) store)]
...
; HELPER FUNCTIONS FOR BOX IMPLEMENTATIONS:
;; !!!
;; Value Store -> (list Value Store)
;; allocate a new box containing v and return it
(define (newbox-value v store) (list (numV 0) empty-store)) ;stub

;; !!!
;; Value Store -> Value
(define (openbox-value v store) (numV 0)) ;stub

;; !!!
;; Value Value Store -> (list Value Store)
(define (setbox-value v1 v2 store) (list (numV 0) (numV 0) empty-store)) ; stub
  

;; interp-hg-acc : HG (envof Value) Store -> (list Value Store)
...
(define (interp-hg-acc hg env store)
  (type-case HG hg
    ...
    [id (x) (list (interp-id x env) store)]
    [fun (x body)
    ...
    ;; IMPLEMENT BOXES!
    ; Just adding these boxes has made us have to change everything else above
    #;[newbox (e) (... (#;fn-for-hg e))]
    [newbox (e) (match-let ([`(,v1 ,store1) (interp-hg-acc e env store)])
                  (newbox-value v1 store1))] ; copied ismtz
    ;; really encourage using helpers: api that hooks things together
    #;[isboxz (e)
              (match-let ([`(,v1 ,store1) (interp-hg-acc e env store)])
    ...
    #;[setbox (e1 e2) (... (#;fn-for-hg e1)
                           (#;fn-for-hg e2))]
    [setbox (e e2) (match-let ([`(,v1 ,store1) (interp-hg-acc e env store)])
                     (match-let ([`(,v2 ,store2) (interp-hg-acc e env store)])
                       (set-box-value v1 v2 store1)))]
    #;[openbox (e) (... (#;fn-for-hg e))]
    [openbox (e) (match-let ([`(,v1 ,store1) (interp-hg-acc e env store)])
                   (list (openbox-value v1 store1) store1))]
    ;; wrote with list just to make it clear not modifying the store
    ;; ie. only returning a balue
    ))
