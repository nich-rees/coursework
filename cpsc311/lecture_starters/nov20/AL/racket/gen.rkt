...
;;
;; gen.rkt - a test case generator for the CPSC411 Applicative Language (AL).
;;           It incorporates a direct-style interpreter to filter programs
;;           that successfully run to completion
;;
...
(define (nf? o) (not (false? o)))


(define (runs-without-error? e)
  (with-handlers ([exn:fail? (lambda (_) #f)])
    (ee e)
    #t))

(define (running-terms n [s 3])
  (filter runs-without-error? (gen-terms n s)))
