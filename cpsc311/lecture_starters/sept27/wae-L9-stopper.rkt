...
;; Some practice for the final, implementing:
;; WAE -> Number
...
(define (interp/wae wae)
  (type-case WAE wae
    ...
    [sub (l r) (- (interp/wae l)
                  (interp/wae r))]
    [with (id named body)
          (interp/wae (subst body id (num (interp/wae named))))]
...
(test/exn (interp/wae (id 'x)) "Unbound identifier")
(test/exn (interp/wae (with 'a (num 4) (id 'x))) "Unbound identifier")
