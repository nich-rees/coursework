...
;; interp. a table for lookin up identifer bindings
...
(define (interp/wae-env wae0)
  ...
  (local [(define (interp/wae-env wae env)
            (type-case WAE wae
              [num (n) n]
              [add (l r) (+ (interp/wae l env)
                            (interp/wae r env))]
              [sub (l r) (- (interp/wae l env)
                            (interp/wae r env))]
              [with (x named body)
                    (let ([vnamed (interp/wae-env named env)]) 
                      (interp/wae-env body        ;; "let" is plai's "with"
                                      (extend-env env x vnamed)))]
                      ;;Env takes num, don't need to preserve num-wae like subst
              [id (x) (error 'interp/wae "Unbound identifier: ~a" x)]))]
    (interp/wae-env wae0 empty-env)))


(test (interp/wae-env WAE1) 4)
(test (interp/wae-env WAE2) 8)
(test (interp/wae-env WAE3) 20)
(test (interp/wae-env WAE4) 14)
(test (interp/wae-env WAE5) 15)
(test (interp/wae-env WAE6) 8)
(test (interp/wae-env WAE7) 10)

(test/exn (interp/wae-env (id 'x)) "Unbound")
