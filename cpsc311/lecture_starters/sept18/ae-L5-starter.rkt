...
(define AE4 (sub AE3 AE2)) ;; examples important to use for function examples... tests
...
; we're using s expressions like they were structs (before structs)
; but this will let us parse
(define (fn-for-aefs sexp)
  (cond [(number? sexp) (... sexp)]
        ...
        [(and (list? sexp)
         ...
         (... (fn-for-aefs (second sexp))
              (fn-for-aefs (third sexp)))]
        [else (... sexp)])) ; don't want the fuzzer to crash the system... exception
; Even if they are using gensym that has a + in it, not the same as '+

; Not actually going to write parsers this way,
; used this to demonstrate s-expression
; Will use what differentiates bsl to plai: pattern matching, but that's next time
...
;; EXERCISE: Provide some more examples
