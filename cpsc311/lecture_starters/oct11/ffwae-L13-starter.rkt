...
(define (eb->ev eb)
  (match eb
    ...
    ;; ^ semantics would be infinite loop, like room-warmer
    [ev ev]))
...
(define (interp-fixFun f x body env)
  ...
  (letrec ([env^ (λ (x0) ; letrec, not let because env^ self-ref
                (if (symbol=? x0 f)
                    (funV x body env^) ;; myself
                    (env x0)))])
    (funV x body env^)))
...
(define (interp/ffwae-env ffwae0)
  ...
  (local [;; EnvValue EnvValue -> EnvValue
          ...
          (define (interp-fix self body env)
            (let ([b (box (blackHole))]) ;; blackHole for error messages
              (let ([env^ (extend-env env self b)])
                (let ([ev (interp-env body env^)])
                  ;; if body doesn't refer to f when first called, returns value
                  (begin ;; should warn you a side effect is about to happen
                    ;; begin sequences if you have side effects
                    (set-box! b ev)
                    ev)))))
