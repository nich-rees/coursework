...
(define (interp-f1wae f1wae fundefs)
  (type-case F1WAE f1wae
    [num (n) n]
    [add (l r) (+ (interp-f1wae l fundefs)
                  (interp-f1wae r fundefs))]
    [sub (l r) (- (interp-f1wae l fundefs)
                  (interp-f1wae r fundefs))]
    [with (x named body)
          (let ([val (interp-f1wae named fundefs)])
            (let ([subst-body (subst-f1wae body x (num val))])
              (interp-f1wae subst-body fundefs)))]
    [id (x) (error 'interp-f1wae "Unbound instance: ~s" x)]
    [app (fun-name arg)
         (let ([fundef (lookup-fundef fun-name fundefs)])
           (let ([val (interp-f1wae arg fundefs)])
             (let ([subst-body (subst-f1wae (fundef-body fundef)
                                            (fundef-arg-name fundef)
                                            (num val))])
               (interp-f1wae subst-body fundefs))))]
    [if0 (p c a)
         (if (zero? (interp-f1wae p fundefs))
             (interp-f1wae c fundefs)
             (interp-f1wae a fundefs))]))

(define (interp-f1wae f1wae0 fundefs)
  ;; Accumulator env is Env
  ;; Invariant: bindings (in inside-out order) of identifiers to values
  ;;            *due to pendinng substitutions*
  (local [(define (interp-f1wae f1wae fundefs env)
            (type-case F1WAE f1wae
              [num (n) n]
              [add (l r) (+ (interp-f1wae l env) ;no substitution, defer sub
                            (interp-f1wae r env))]
              ...
              [with (x named body)
                    (let ([val (interp-f1wae named fundefs)])
                      ...
                      (interp-f1wae body (extend-env env x val)))] ; update acc
              [id (x) (with-handlers ([exn:fail?
                                       (λ (_)
                                         (error "Unbound identifer ~a" x))])
                        (lookup-env env x))] ; exploit accumulator
              [app (fun-name arg)
                   (let ([fundef (lookup-fundef fun-name fundefs)])
                     (let ([val (interp-f1wae arg)])
                       #;(let ([subst-body (subst-f1wae (fundef-body fundef)
                                                      (fundef-arg-name fundef)
                                                      (num val))])
                         (interp-f1wae subst-body))
                       (interp-f1wae (fundef-body fundef)
                                     (extend-env empty-env ; notably not env
                                      (fundef-arg-name fundef) val))
                       ;; because don't sub into body, only need to check param?
                                     #;(... env))))]
              [if0 (p c a)
                   (if (zero? (interp-f1wae p))
                       (interp-f1wae c)
                       (interp-f1wae a))]))]
    (interp-f1wae f1wae0 empty-env))) ; set accumulator
