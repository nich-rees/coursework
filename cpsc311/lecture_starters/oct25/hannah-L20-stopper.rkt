...
(define (fn-for-binding b)
  (match b
    [(box l) #:when (location? l) (fn-for-location l)]
...
;; Symbol Env Store -> Value
;; produce the value currently associated with the variable x
;; Effect: signal an error in case of a black hole
...
;; Binding -> Location
;; produce the location associated with the given binding
;; Effect: signals an error if b is a black hole
(define (binding->loc b)
  (match b
    [(box l) #:when (location? l) l]
    [(box bh) #:when (BlackHole? bh) (error 'divergence "Nooooooooooooo.....!")]
    [l l]))

;; Hvar Value Env Store -> (list Value Store)
;; assign the variable x to the value v
;; Effect: signals an error if x is unbound.
(define (setvar-value x v env store)
  (let* ([b (lookup-env env x)]
         [l (binding->loc b)]
...
(test (interp-sexp '{with {f {fix x {fun {y} {seqn {setvar x 7} x}}}}
                          {f 2}})
      (numV 7))
