            ...
            (let ([doru/wt (doru->doru/wt doru tenv)])
              (begin
                (unless (equal? (doru/wt-type doru/wt) type)
                  (error 'doru->doru/wt "type mismatch"))
                doru/wt)))
          (define (doru->doru/wt doru tenv)
            (type-case DORU doru
              [num (n) (num/wt tenv n)]
              ...
              [sub (lhs rhs) (... (doru->doru/wt lhs (... tenv))
                                  (doru->doru/wt rhs (... tenv)))]
              [eqwal (lhs rhs) (... (doru->doru/wt lhs (... tenv))
                                    (doru->doru/wt rhs (... tenv)))]
              [lt (lhs rhs) (... (doru->doru/wt lhs (... tenv))
                                 (doru->doru/wt rhs (... tenv)))]
              [bif (pred conseq altern) (... (doru->doru/wt pred (... tenv))
                                             (doru->doru/wt conseq (... tenv))
                                             (doru->doru/wt altern (... tenv)))]
              [with (x named body) (... x
                                        (doru->doru/wt named (... tenv))
                                        (doru->doru/wt body (... tenv)))]
              [id (x)
                  (begin
                    (unless (in-env? tenv x)
                      (error 'doru->doru/wt "~s ∉ ~s" x tenv))
                    (id/wt tenv x))]))]
    (doru->doru/wt doru tenv)))
