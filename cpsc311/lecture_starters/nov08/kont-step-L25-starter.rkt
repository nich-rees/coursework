...
;; replace free instances of k with a continuationn (represents the rest of the
;; computation)
'{+ 5 {throwcc {kont •} 3}}

;; replace the rest of the current computation (i.e. the current continuation)
;; with the continuation argument to throwcc and plug it with the value argument
;; to throwcc
'3
...
;; Ron figuring out what the captured continuation is
;;'{kont {+ 5 •}}
...
;; reinstate the captured continuaton using the given value
'{+ 5 3}

'8


;; What happens if we reference k but not in throwcc
'{+ {letcc {k} k} 7}

;; {kont {+ • 7}}
'{+ {kont {+ • 7}} 7}
;; Error: bad number: {kont {+ • 7}}
...
'{if0 0
      22
      {+ 0 {throwcc 0 0}}} ;uh oh, throwing a 0? but won't go into this branch
