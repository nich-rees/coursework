...
;; TL;DR:
;; Problem 1a (line 102): design a helper to interpret match/pair expressions
;; Problem 1b (line 120): add support for with as sugar to the SPL parser.
...
;; you must put them after line 167 or PLAI will likely give errors.
...
(define (interp-match/pair v x1 x2 body env)
  (if (and (identifier? x1) (identifier? x2))
      (if (pair? (interp/spl v))
          (interp/spl body (extend-env
                            (extend-env x1 (first v))
                            x2 (second v)))
          (error "Not pair with match"))
...
(define (with x named body)
  (match/pair (pair (interp/spl named) 0)
              (pair x (gensymb)) body))
