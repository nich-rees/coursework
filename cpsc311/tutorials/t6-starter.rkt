...
#;(define (interp/bs bs0)
    ...
    (local [;; Value Value -> Value
            ...
            (define (fn-for-bs f env bstopV)
              (type-case BS f
                [num (n) (numV n)]
                [add (l r) (add/bs (fn-for-bs l env bstopV)
                                   (fn-for-bs r env bstopV))]
                [sub (l r) (sub/bs (fn-for-bs l env bstopV)
                                   (fn-for-bs r env bstopV))]
                [id (x) (id/bs x env bstopV)]
                [fun (x body) (funV x body env)]
                [app (rator rand)
                     (apply/bs (fn-for-bs rator env bstopV)
                               (fn-for-bs rand env bstopV)
                               bstopV)]
                [backstop (bs body)
                          (let ([bstopV^ (fn-for-bs bs env bstopV)])
                            (fn-for-bs body env bstopV^))]))]
      (fn-for-bs bs0 empty-env #f)))
...
(define (interp/bs bs0)
  ...
  ;;            of identifiers to values due to deferred substitutions
  ;; Accumulator: bstopV is MaybeValue
  ;; Invariant: dynamically most recent backstop in bs0 at bs, or #f if none
  (local [;; Value Value -> Value
          (define (apply/bs vrator vrand bstopV)
            (with-value-as-funv vrator
              (λ (x body env) ;; vrator = (funV x ev-body env)
          ...
          ;; BS Env -> (list Value MaybeValue)
          (define (fn-for-bs f env bstopV)
            (type-case BS f
              [num (n) (list (numV n) bstopV)]
              [add (l r) (match-let ([`(,vl ,bstopV1) (fn-for-bs l env bstopV)])
                           (match-let ([`(,vr ,bstopV2)
                                        (fn-for-bs r env bstopV1)])
                             (list (add/bs vl vr) bstopV2)))]
              [sub (l r) (match-let ([`(,vl ,bstopV1) (fn-for-bs l env bstopV)])
                           (match-let ([`(,vr ,bstopV2)
                                        (fn-for-bs r env bstopV1)])
                             (list (sub/bs vl vr) bstopV2)))]
              [id (x) (list (id/bs x env bstopV) bstopV)]
              [fun (x body) (list (funV x body env) bstopV)]
              [app (rator rand)
                   (match-let ([`(,vrat ,bstopV1) (fn-for-bs rator env bstopV)])
                     (match-let ([`(,vand ,bstopV2)
                                  (fn-for-bs rand env bstopV1)])
                       (apply/bs vrat vand bstopV2)))]
              [backstop (bs body)
                        (let ([bstopV^ (car (fn-for-bs bs env bstopV))])
                          (fn-for-bs body env bstopV^))]))]
    (match-let ([`(,val ,bstopV) (fn-for-bs bs0 empty-env #f)])
      val)))
...
(test (interp/bs (add (backstop (num 7) (num 5)) (id 'x))) (numV 12))