...
;; adapts the given substitution to produce (emptyType) in place of any variables
(define (finalize-subst subst)
  (compose-subst (const-subst (emptyType)) subst))
...
;; produces a substitution of types that solves the given system of equations
;; postcondition: for each equation t1 = t2,
     ...
     (let ([eqn (first soeqns)]
       ...
       (match eqn
         ...
         [(assert-equal (numType) (numType))
          (unify--with-subst soeqns^)]
         [(assert-equal (funType arg1 res1) (funType arg2 res2))  ; (arg1 -> res1) = (arg2 -> res2)
