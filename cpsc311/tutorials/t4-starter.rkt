  ...
  (local [;; Split Natural -> (list Natural Natural)
          ...
          (define (split-score--acc s factor)
            (cond [(empty? s) (list 0 factor)]
                  [(number? s) (list (* s factor) (if (= 5 s)
                                                      (* factor 2)
                                                      factor))]
                  [else ; (cut? s)
                   (match-let ([`(,suml ,factorl)
                                (split-score--acc (cut-first s) factor)])
                     (match-let ([`(,sumr ,factorr)
                                  (split-score--acc (cut-second s) factorl)])
                       (list (+ suml sumr) factorr)))]))]
    ;; 1 = (expt 2 0)
            ...
            (cond [(empty? s) 0]
                  [(number? s) (let ([val (* s factor)])
                                 (begin
                                   ...
                                   val))]
                  [else ; (cut? s)
                   (let* ([val1 (split-score--acc (cut-first s))]
                          [val2 (split-score--acc (cut-second s))])
