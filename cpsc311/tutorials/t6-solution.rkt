...
;; BS BS -> BS
;; desugar a seqn expression into an equivalent BS expression
(define (seqn expr1 expr2) (with (gensym) expr1 expr2))


;; BS BS -> BS
;; desugar an old-style scopey backstop into a sequence expr
(define (scopey-backstop expr1 expr2) (seqn (backstop expr1) expr2))

;; Examples stolen from above.  Add more as needed.
(define BS1 (id 'x))

(define BS2 (scopey-backstop (num 7)
                             (id 'x)))

(define BS3
  (scopey-backstop (num 7)
                   (add (id 'x) (id 'y))))

(define BS4
  (scopey-backstop (num 7)
                   (add (id 'x) (scopey-backstop (num 8) (id 'y)))))
            ...
            (type-case BS f
              ...
              [backstop (bs)
                        (match-let ([(list new-bstopV bstopV^)
                          ...
                          ;; What return value?!? 
                          ;; option 1) backstop in place at beginning of entire
                          ;;    expression
                          ;; option 2) backstop in place at end of evaluating
                          ;;    the argument to backstop
                          ;; (we go with option 1)
                          (list bstopV new-bstopV))]))]
...
(test (interp/bs (add (scopey-backstop (num 7) (num 9)) (id 'x))) (numV 16))
(test/exn (interp/bs (add (id 'x) (scopey-backstop (num 7) (num 9)))) "Unbound")