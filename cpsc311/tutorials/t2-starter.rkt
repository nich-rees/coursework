...
;; Env is (listof (list Symbol Number))
;; interp. a collection of deferred substitutions
...
;; Env Symbol Number -> Env
;; extend env with a binding from x to v
...
#;(define (lookup-env env x)
  (cond [(empty? env) (error 'lookup-env "Unbound variable: ~a" x)]
        [(symbol=? x (caar env)) (cadar env)]
        [else (lookup-env (cdr env) x)]))
...
(define-type Env
  [mt-env]
  [nonempty-env (x symbol?) (value number?) (tail Env?)])

;; Problem 4: Redesign empty-env, extend-env, and lookup-env to use this Env 

;; Env
;; the empty environment
(define empty-env (mt-env))

;; Env Symbol Number -> Env
;; extend env with a binding from x to v
(define (extend-env env x v)
  (nonempty-env x v env)) ;; apparently can implement this in Env type
...
(define (lookup-env env x)
  (type-case Env env
    [mt-env () (error 'lookup-env "Unbound variable: ~a" x)]
    [nonempty-env (x^ v^ env^)
                  (if (symbol=? x x^)
                      v^
                      (lookup-env env^ x))]))
