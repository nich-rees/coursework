...
;; Produces the first identifier name that the given name maps to if any, otherwise itself.
...
;; Obfuscates a WAE program's names and produces its deobfuscation table (i.e. NEW to OLD names).
(define (obfuscate/wae wae)
  ;; Accumulator: mappings is Mappings
  ;; Invariant: Maps variables bound in context to their obfuscated name (i.e. OLD to NEW names).
  (local [;; WAE Mappings -> (list WAE Mappings)
          (define (obfuscate/wae wae mappings)
            (type-case WAE wae
              [num/wae (n)
                       (list (num/wae n) empty)]
              [add/wae (l r)
                       (match-let* ([(list obs-l dt-l) (obfuscate/wae l mappings)]
                                    [(list obs-r dt-r) (obfuscate/wae r mappings)])
                         (list (add/wae obs-l obs-r)
                               (append dt-l dt-r)))]
              [sub/wae (l r)
                       (match-let* ([(list obs-l dt-l) (obfuscate/wae l mappings)]
                                    [(list obs-r dt-r) (obfuscate/wae r mappings)])
                         (list (sub/wae obs-l obs-r)
                               (append dt-l dt-r)))]
              [with/wae (id named body)
                        (let ([obs-id (gensym)])
                          (match-let* ([(list obs-named dt-named) (obfuscate/wae named mappings)]
                                       [(list obs-body dt-body) (obfuscate/wae body (extend-mappings id obs-id mappings))])
                            (list (with/wae obs-id obs-named obs-body)
                                  (extend-mappings obs-id id (append dt-named dt-body)))))]
              [id/wae (x)
                      (list (id/wae (resolve-mapping x mappings))
                            empty)]))]
