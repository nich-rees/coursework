...
(define (interp/bs bs0)
  ...
  (local [;; Value Value MaybeValue -> Value
          ...
          ;; BS Env MaybeValue -> Value
          (define (fn-for-bs f env bstopV)
            (type-case BS f
              [num (n) (numV n)]
              [add (l r)
                   (let* ([lv (fn-for-bs l env bstopV)]
                          [rv (fn-for-bs r env bstopV)])
                     (add/bs lv rv))]
              [sub (l r)
                   (let* ([lv (fn-for-bs l env bstopV)]
                          [rv (fn-for-bs r env bstopV)])
              ...
              [app (rator rand)
                   (let* ([ratorv (fn-for-bs rator env bstopV)]
                          [randv (fn-for-bs rand env bstopV)])
