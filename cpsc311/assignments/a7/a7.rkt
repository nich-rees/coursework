            ...
            (let ([tfwae/wt (tfwae->tfwae/wt tfwae tenv)])
              (begin
                (unless (equal? (tfwae/wt-type tfwae/wt) type)
                  (error 'tfwae->tfwae/wt "Type mismatch: ~s ≠ ~s"
                         (tfwae/wt-type tfwae/wt) type))
                tfwae/wt)))
                   ...
                   (let* ([rand/wt (tfwae->tfwae/wt rand tenv)]
                     ...
                     (begin
                       (unless (funType? ratortype)
                         (error 'tfwae->tfwae/wt
                                "Type mismatch: ~s is not funType" ratortype))
                       (unless (equal? (funType-arg ratortype)
                                       (tfwae/wt-type rand/wt))
                         (error 'tfwae->tfwae/wt "Type mismatch: ~s ≠ ~s"
                                (funType-arg ratortype)
                                (tfwae/wt-type rand/wt)))
...
(test (tfwae->tfwae/wt
      ...
      (app/wt (fun/wt 'x (numType) (numType)
              ...
              (num/wt '() 6)))
;; Apologizes for the length of the example below...
;; I came up with it to show how functions are first class,
;; but the tenvs made the tfwae/wt version very very long,
;; but when I realized this I was already in too deep
(test (tfwae->tfwae/wt
       (app (with 'f
                  (funType (funType (numType) (numType))
                           (funType (numType) (numType)))
...
(test/exn (tfwae->tfwae/wt
          ...
          "Type mismatch")
(test/exn (tfwae->tfwae/wt (id 'x) '()) "")
(test/exn (tfwae->tfwae/wt (with 'x (funType (numType) (numType)) (num 2)
                                 (id 'x)) '())
          "Type mismatch")
(test/exn (tfwae->tfwae/wt (app (num 7) (num 7)) '()) "Type mismatch")
(test/exn (tfwae->tfwae/wt (app (fun 'f
                                     (funType (numType) (numType))
                                     (numType)
                                     (num 7))
                                (num 7))
...
(define (certify-tfwae tfwae/wt tfwae)
  (begin
    (let ([tenv (tfwae/wt-tenv tfwae/wt)]
      ...
      (begin
        (unless (equal? '() tenv)
          (error 'check-tfwae/wt "Bad program environment: ~s" tenv)))
      (unless (equal? tfwae tfwae^)
        (error 'check-tfwae/wt "Program mismatch: ~s ≠ ~s" tfwae tfwae^)))))


(test (certify-tfwae (num/wt '() 7) (num 7)) (void))
(test (certify-tfwae (add/wt (num/wt '() 3) (num/wt '() 4))
                     (add (num 3) (num 4)))
      (void))
(test (certify-tfwae (sub/wt (num/wt '() 4) (num/wt '() 3))
                     (sub (num 4) (num 3)))
      (void))
(test (certify-tfwae (with/wt 'x (numType) (num/wt '() 7)
                              (id/wt (list (list 'x (numType))) 'x))
                     (with 'x (numType) (num 7) (id 'x)))
      (void))
(test (certify-tfwae (with/wt 'x (numType)
                              (sub/wt (num/wt '() 5) (num/wt '() 3))
                              (add/wt (id/wt (list (list 'x (numType))) 'x)
                                      (num/wt (list (list 'x (numType))) 5)))
                     (with 'x (numType) (sub (num 5) (num 3))
                           (add (id 'x) (num 5))))
      (void))
(test (certify-tfwae
       (app/wt (fun/wt 'x (numType) (numType)
...
(test/exn (certify-tfwae (num/wt (list (list 'x (numType))) 7) (num 7))
          "Bad program environment")
(test/exn (certify-tfwae (id/wt (list (list 'x (numType))) 'x) (id 'x))
          "Bad program environment")
(test/exn (certify-tfwae (num/wt '() 7) (add (num 3) (num 4)))
          "Program mismatch")
(test/exn (certify-tfwae (num/wt (list (list 'x (numType))) 7)
                         (add (num 3) (num 4)))
          "Bad program environment")
...
(test (typecheck-tfwae (num 7)) (num 7))
(test (typecheck-tfwae (add (num 3) (num 4))) (add (num 3) (num 4)))
(test (typecheck-tfwae (sub (num 4) (num 3))) (sub (num 4) (num 3)))
(test (typecheck-tfwae (with 'x (numType) (num 7) (id 'x)))
      (with 'x (numType) (num 7) (id 'x)))
(test (typecheck-tfwae (with 'x (numType) (sub (num 5) (num 3))
                             (add (id 'x) (num 5))))
      (with 'x (numType) (sub (num 5) (num 3)) (add (id 'x) (num 5))))
(test (typecheck-tfwae
       (app (fun 'x (numType) (numType) (add (id 'x) (num 1))) (num 6)))
      (app (fun 'x (numType) (numType) (add (id 'x) (num 1))) (num 6)))
(test (typecheck-tfwae
       (app (with 'f
                  (funType (funType (numType) (numType))
...
(test/exn (typecheck-tfwae (add (fun 'x (numType) (numType) (id 'x)) (num 5)))
          "Type mismatch")
(test/exn (typecheck-tfwae (add (num 4) (fun 'x (numType) (numType) (id 'x))))
          "Type mismatch")
(test/exn (typecheck-tfwae (sub (fun 'x (numType) (numType) (id 'x)) (num 3)))
          "Type mismatch")
(test/exn (typecheck-tfwae (sub (num 2) (fun 'x (numType) (numType) (id 'x))))
          "Type mismatch")
(test/exn (typecheck-tfwae (id 'x)) "")
(test/exn (typecheck-tfwae (with 'x (funType (numType) (numType)) (num 2)
                                 (id 'x)))
          "Type mismatch")
(test/exn (typecheck-tfwae (app (num 7) (num 7))) "Type mismatch")
(test/exn (typecheck-tfwae (app (fun 'f
                                     (funType (numType) (numType))
                                     (numType)
                                     (num 7))
                                (num 7)))
          "Type mismatch")
(test/exn (typecheck-tfwae (fun 'f
                                (numType)
                                (funType (numType) (numType))
                                (num 7)))
          "Type mismatch")
