...
(define (apply-k k v)
  (match k
    ...
    [`(extend-k-addr ,k^ ,v^)
     (apply-k k^ (add/kra v^ v))]
    [`(extend-k-addl ,k^ ,kra^ ,env^)
     (interp/kra-env/kd kra^ env^ (extend-k-addr k^ v))]
    [`(extend-k-if ,k^ ,krac^ ,kraa^, env^)
     (if (value->bool v)
         (interp/kra-env/kd krac^ env^ k^)
         (interp/kra-env/kd kraa^ env^ k^))]
    [`(extend-k-appran ,k^ ,v^)
     (apply/kra/kd v^ v k^)]
    [`(extend-k-apprat ,k^ ,kra^ ,env^)
     (interp/kra-env/kd kra^ env^ (extend-k-appran k^ v))]))
...
(define (apply/kra/kd v1 v2 k)
  ...
  (type-case Value v1
    ...
    [else (error 'apply/kra/kd "Bad function: ~a" v1)]))



;; KRA Env Continuation -> Value
...
(define (interp/kra-env/kd kra env k)
  ...
  (type-case KRA kra
    [num (n) (apply-k k (numV n))]
    [add (l r) (interp/kra-env/kd l env (extend-k-addl k r env))]
    [nought? (e) (interp/kra-env/kd e env (extend-k-nought k))]
    [bool (b) (apply-k k (boolV b))]
    [ifB (p c a) (interp/kra-env/kd p env (extend-k-if k c a env))]
    [id (x) (apply-k k (id/kra x env))]
    [fixFun (f x body) (apply-k k (fixFun/kra f x body env))]
    [app (rator rand)
         (interp/kra-env/kd rator env (extend-k-apprat k rand env))]))


;; KRA -> Value
;; interpret the given KRA expression
;; EFFECTS: Signals an error in case of runtime type error.
(define (interp/kra/kd kra0)
  (interp/kra-env/kd kra0 empty-env empty-k)) ; stub for the examples
