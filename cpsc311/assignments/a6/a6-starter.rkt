...
(define (apply/kra v1 v2)
  (type-case Value v1
    [funV (x body env)
          (interp/kra-env body (extend-env env x v2))]
...
(define (interp/kra-env kra env)
  (type-case KRA kra
    [num (n) (numV n)]
    [add (l r)
         (add/kra (interp/kra-env l env)
                  (interp/kra-env r env))]
    [nought? (e) (nought?/kra (interp/kra-env e env))]
    [bool (b) (boolV b)]
    [ifB (p c a)
         (if (value->bool (interp/kra-env p env))
             (interp/kra-env c env)
             (interp/kra-env a env))]
    [id (x) (id/kra x env)]
    [fixFun (f x body) (fixFun/kra f x body env)]
    [app (rator rand)
         (apply/kra (interp/kra-env rator env)
                    (interp/kra-env rand env))]))


;; KRA -> Value
;; interpret the given KRA expression
;; EFFECTS: Signals an error in case of runtime type error.
(define (interp/kra kra)
  (interp/kra-env kra empty-env)) ; stub for the examples
