...
;; Thunk is one of:
;; - (nought-k-th ,Value ,Kont)
;; - (add2-k-th ,Value ,KRA ,Env ,Kont)
;; - (add-k-th ,Value ,Value ,Kont)
;; - (ifc-k-th ,KRA ,Env ,Kont)
;; - (ifa-k-th ,KRA ,Env ,Kont)
;; - (app2-k-th ,Value ,KRA ,Env ,Kont)
;; - (apply-k-th ,Value ,Value ,Kont)
;; - (interp-body-th ,KRA ,Env ,KID ,Value ,Kont)
;; - (interp-num-th ,Number ,Kont)
;; - (interp-add-th ,KRA ,KRA ,Env ,Kont)
;; - (interp-nought-th ,KRA ,Env ,Kont)
;; - (interp-bool-th ,Boolean ,Kont)
;; - (interp-ifB-th ,KRA ,KRA ,KRA ,Env ,Kont)
;; - (interp-id-th ,KID ,Env ,Kont)
;; - (interp-fixFun-th ,KID ,KID ,KRA ,Env ,Kont)
;; - (interp-app-th ,KRA ,KRA ,Env ,Kont)
;; - (interp/kra-th ,KRA)
;; interp.  suspended interpretation computations

;; Any -> Boolean
(define (thunk? th)
  (match th
    [`(nought-k-th ,v ,k) #t]
    [`(add2-k-th ,vl ,r ,env ,k) #t]
    [`(add-k-th ,vl ,vr ,k) #t]
    [`(ifc-k-th ,c ,env ,k) #t]
    [`(ifa-k-th ,a ,env ,k) #t]
    [`(app2-k-th ,vrator ,rand ,env ,k) #t]
    [`(apply-k-th ,vrator ,vrand ,k) #t]
    [`(interp-body-th ,body ,env ,x ,v2 ,k) #t] ;;;;;
    [`(interp-num-th ,n ,k) #t]
    [`(interp-add-th ,l ,r ,env ,k) #t]
    [`(interp-nought-th ,e ,env ,k) #t]
    [`(interp-bool-th ,b ,k) #t]
    [`(interp-ifB-th ,p ,c ,a ,env ,k) #t]
    [`(interp-id-th ,x ,env ,k) #t]
    [`(interp-fixFun-th ,f ,x ,body ,env ,k) #t]
    [`(interp-app-th ,rator ,rand ,env ,k) #t]
    [`(interp/kra-th ,kra0) #t]
    [else #f]))


;; Thunk -> (trampolineof Value)
(define (apply-th th)
  (match th
    [`(nought-k-th ,v ,k)
     (apply-k k (nought?/kra v))]
    [`(add2-k-th ,vl ,r ,env ,k)
     (interp/kra-env/kdtd
      r env
      (add2-k vl k))]
    [`(add-k-th ,vl ,vr ,k)
     (apply-k k (add/kra vl vr))]
    [`(ifc-k-th ,c ,env ,k)
     (interp/kra-env/kdtd c env k)]
    [`(ifa-k-th ,a ,env ,k)
     (interp/kra-env/kdtd a env k)]
    [`(app2-k-th ,vrator ,rand ,env ,k)
     (interp/kra-env/kdtd
      rand env
      (app2-k vrator k))]
    [`(apply-k-th ,vrator ,vrand ,k)
     (apply/kra/kdtd vrator vrand k)]
    [`(interp-body-th ,body ,env ,x ,v2 ,k)
     (interp/kra-env/kdtd body (extend-env env x v2) k)]
    [`(interp-num-th ,n ,k)
     (apply-k k (numV n))]
    [`(interp-add-th ,l ,r ,env ,k)
     (interp/kra-env/kdtd
      l env
      (add1-k r env k))]
    [`(interp-nought-th ,e ,env ,k)
     (interp/kra-env/kdtd
      e env
      (nought-k k))]
    [`(interp-bool-th ,b ,k)
     (apply-k k (boolV b))]
    [`(interp-ifB-th ,p ,c ,a ,env ,k)
     (interp/kra-env/kdtd
      p env
      (if-k c a env k))]
    [`(interp-id-th ,x ,env ,k)
     (apply-k k (id/kra x env))]
    [`(interp-fixFun-th ,f ,x ,body ,env ,k)
     (apply-k k (fixFun/kra f x body env))]
    [`(interp-app-th ,rator ,rand ,env ,k)
     (interp/kra-env/kdtd
      rator env
      (app1-k rand env k))]
    [`(interp/kra-th ,kra0)
     (interp/kra-env/kdtd kra0 empty-env initial-k)]))

(define (nought-k-th v k)
  `(nought-k-th ,v ,k))

(define (add2-k-th vl r env k)
  `(add2-k-th ,vl ,r ,env ,k))

(define (add-k-th vl vr k)
  `(add-k-th ,vl ,vr ,k))

;; NOTICE that the next two functions are actually the same function!
;; We could combine them into one "interp-th" function, or really one
(define (ifc-k-th c env k)
  `(ifc-k-th ,c ,env ,k))

(define (ifa-k-th a env k)
  `(ifa-k-th ,a ,env ,k))

(define (app2-k-th vrator rand env k)
  `(app2-k-th ,vrator ,rand ,env ,k))

(define (apply-k-th vrator vrand k)
  `(apply-k-th ,vrator ,vrand ,k))

;; from apply

(define (interp-body-th body env x v2 k)
  `(interp-body-th ,body ,env ,x ,v2 ,k))

;; from interp/kra

(define (interp-num-th n k)
  `(interp-num-th ,n ,k))

(define (interp-add-th l r env k)
  `(interp-add-th ,l ,r ,env ,k))

(define (interp-nought-th e env k)
  `(interp-nought-th ,e ,env ,k))

(define (interp-bool-th b k)
  `(interp-bool-th ,b ,k))

(define (interp-ifB-th p c a env k)
  `(interp-ifB-th ,p ,c ,a ,env ,k))

(define (interp-id-th x env k)
  `(interp-id-th ,x ,env ,k))

(define (interp-fixFun-th f x body env k) 
  `(interp-fixFun-th ,f ,x ,body ,env ,k))

(define (interp-app-th rator rand env k)
  `(interp-app-th ,rator ,rand ,env ,k))

(define (interp/kra-th kra0)
  `(interp/kra-th ,kra0))
;;;;
...
(define (fn-for-trampoline t)
  (type-case trampoline t
    [bounce (p) (... (fn-for-trampoline (apply-th p)))]
    [dismount (v) (... v)]))
...
(define (apply-k k0 v)
  (match k0
    ...
    [`(nought-k ,k)
     (bounce (nought-k-th v k))]
    [`(add1-k ,r ,env ,k)
     (bounce
      (add2-k-th v r env k))]
    [`(add2-k ,vl ,k)
     (bounce (add-k-th vl v k))]
    [`(if-k ,c ,a ,env ,k)
     (if (value->bool v)
         (bounce (ifc-k-th c env k))
         (bounce (ifa-k-th a env k)))]
    [`(app1-k ,rand ,env ,k)
     (bounce
      (app2-k-th v rand env k))]
    [`(app2-k ,vrator ,k)
     (bounce (apply-k-th vrator v k))]))
...
(define (apply/kra/kdtd v1 v2 k)
  (type-case Value v1
    [funV (x body env)
          (bounce (interp-body-th body env x v2 k))]
...
(define (interp/kra-env/kdtd kra env k)
  (type-case KRA kra
    [num (n) (bounce (interp-num-th n k))]
    [add (l r) (bounce (interp-add-th l r env k))]
    [nought? (e) (bounce (interp-nought-th e env k))]
    [bool (b) (bounce (interp-bool-th b k))]
    [ifB (p c a) (bounce (interp-ifB-th p c a env k))]
    [id (x) (bounce (interp-id-th x env k))]
    [fixFun (f x body) (bounce (interp-fixFun-th f x body env k))]
    [app (rator rand) (bounce (interp-app-th rator rand env k))]))
