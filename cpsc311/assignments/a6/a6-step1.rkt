...
(define (apply/kra/k v1 v2 k)
  ...
  (type-case Value v1
    ...
    [else (error 'apply/kra/k "Bad function: ~a" v1)]))
...
(define (interp/kra-env/k kra env k)
  ...
  (type-case KRA kra
    [num (n) (k (numV n))]
    [add (l r)
         (interp/kra-env/k l env
                           (λ (v)
                             (interp/kra-env/k r env
                                               (λ (v^) (k (add/kra v v^))))))]
    [nought? (e) (interp/kra-env/k e env (λ (v) (k (nought?/kra v))))]
    [bool (b) (k (boolV b))]
    [ifB (p c a)
         (interp/kra-env/k p env
                           (λ (v) (if (value->bool v)
                                      (interp/kra-env/k c env k)
                                      (interp/kra-env/k a env k))))]
    [id (x) (k (id/kra x env))]
    [fixFun (f x body) (k (fixFun/kra f x body env))]
    [app (rator rand)
         (interp/kra-env/k
          rator env
          (λ (ratorv)
            (interp/kra-env/k rand env
                              (λ (randv)
                                (apply/kra/k ratorv randv k)))))]))


;; KRA -> Value
;; interpret the given KRA expression
;; EFFECTS: Signals an error in case of runtime type error.
(define (interp/kra/k kra0)
  (interp/kra-env/k kra0 empty-env identity)) ; stub for the examples
