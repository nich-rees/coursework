...
;; 
;; (let-path/eff ([path]) e) - bind f to the current accumulator and evalute e
;; (with-path/eff path e)  - run e with the accumulator set to env for its
;;                           dynamic extent

;; 
;; (let-ovr/eff ([ovr]) e) - bind f to the current accumulator and evalute e
;; (with-ovr/eff ovr e)  - run e with the accumulator set to env for its dynamic
;;                         extent
...
;; Computation is Env Path OverrideList -> XTValue
...
;; run c in a context where the reference context is set to path
...
(define (run/eff c env0 path0 ovr0)
  (c env0 path0 ovr0))
...
(define (interp-with x named body)
  ...
  (let ([xtd (thunkxtb/xt named env)])
    (with-env/eff (extend-env env x xtd)
      (interp/eff body)))))
           ...
           (let-env/eff
            ...
            (match-let ([(thunkxtb/xt xtb env^)
                         (with-handlers
                             ([exn:fail?
                               (λ (_)
                                 (error 'interp "Unbound identifier: ~a" x))])
                           (lookup-env env x))])
              (with-env/eff env^
  ...
  (type-case Xamot xt
    ...
    [and/xt (lhs rhs)
            (let/eff* ([vl (interp/eff lhs)]
                       [vr (interp/eff rhs)])
                      (return/eff (string-append vl vr)))]
    [nil/xt () (return/eff "")]
