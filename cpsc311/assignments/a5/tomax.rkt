...
(define (TODO) (error 'todo "MUST BE IMPLEMENTED"))
...
;; -> Computation
(define (fail/eff) (λ (input) empty-stream))
...
(define (pop/eff mc p)
  (λ (input)
    (if (or (false? mc) (char=? mc (first input)))
        ((return/eff (p (first input))) (rest input))
        ((fail/eff) input))))
...
(define-syntax let/eff
  (syntax-rules ()
    [(_ ([x c1]) c2)
     (λ (input)
       (stream-flatten (stream-map (λ (x) (c2 (second x))) (c1 input))))]))
