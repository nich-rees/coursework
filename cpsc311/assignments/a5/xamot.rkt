...
;; (let-env/eff ([env]) e) - bind f to the current env accumulator and evalute e
;; (let-path/eff ([path]) e) - bind f to the current path accumulator
;;                             and evalute e
;; (let-ovr/eff ([ovr]) e) - bind f to the current ovr accumulator and evalute e
;; (with-env/eff env e)  - run e with the accumulator set to env for its dynamic
;;                         extent
;; (with-path/eff env e)  - run e with the accumulator set to path for
;;                          its dynamic extent

;; The Generic Interface Components:
;; (return/eff e) - return the value of e
;; (run/eff e env0 path0 ovr) - run computation e, starting with accumulators
;;                              env0, path0, and ovr
;; (let/eff ([x e1] e2)) - bind x to the value of e1 in e2, threading path
...
;; Computation is Env Path OverrideList -> (list Path XTValue)
;; interp. a function that awaits two standard acc and a threaded acc
;;         and produces a value and an updated path

(define-syntax let-env/eff
  (syntax-rules ()
    [(_ ([env]) c)
     (λ (env path ovr) (c env path ovr))]))

(define-syntax let-path/eff
  (syntax-rules ()
    [(_ ([path]) c)
     (λ (env path ovr) (c env path ovr))]))

(define-syntax let-ovr/eff
  (syntax-rules ()
    [(_ ([ovr]) c)
     (λ (env path ovr) (c env path ovr))]))
...
(define (with-env/eff env c)
  (λ (env0 path ovr) (c env path ovr)))

;; Path Computation -> Computation
;; run c in a context where the path is set to path
(define (with-path/eff path c)
  (λ (env path0 ovr) (c env path ovr)))
...
(define-syntax return/eff
  (syntax-rules ()
    [(_ v) (λ (env path ovr) (list path v))]))

;; Computation Env Path OverrideList -> XTValue
;; run the given computation and produce its result
(define-syntax run/eff
  (syntax-rules ()
    [(_ c env0 pi ovr)
     (match-let ([`(,po ,v) (c env0 pi ovr)])
       v)]))
...
;; XTValue XTValue -> Computation
(define (append-value v1 v2)
  (return/eff (string-append v1 v2)))

;; XTID Xamot Xamot -> Computation
;; interpret the expression (with x named body)
(define (interp-with x named body)
  (let-env/eff ([env])
               (with-env/eff (extend-env env x named)
                 (interp/eff body))))

;; XTID (XTBinding -> Xamot) -> Computation
;; interpret a reference, either as an override, if present, or relevant binding
(define (interp-ref x resolve-xtb)
  (let-path/eff
   ([path])
   (let-ovr/eff
    ([ovr])
           ...
           (let-env/eff
            ...
            (let ([xtb
                   (with-handlers
                     ...
                     (lookup-env env x))])
              (interp/eff (resolve-xtb xtb))))]))))

;; XTID -> Computation
;; interpret an identifier ref: its override, if present, otherwise its binding
(define (interp-id/xt x)
  (let-path/eff ([path])
                (with-path/eff (cons (id/xt x) path)
                  (interp-ref x (λ (xtb) (binding-id xtb x))))))


;; XTID Natural -> Computation
;; interpret an array ref: its override, if present, otherwise its binding
(define (interp-aref/xt x n)
  (let-path/eff ([path])
                (with-path/eff (cons (aref/xt x n) path)
                  (interp-ref x (λ (xtb) (binding-aref xtb x n))))))


;; Xamot -> Computation
(define (interp/eff xt)
  (type-case Xamot xt
    [char/xt (c) (return/eff (string c))]
    [and/xt (lhs rhs)
            (let/eff* ([vl (interp/eff lhs)]
                       [vr (interp/eff rhs)])
                      ;; I'm pretty sure this is the root of my failing test
                      ;; cases. vr does not get subbed out for a value in
                      ;; the override list I think...
                      ;; (see my failing cases by crtl-f !!)
                      ;; so something to do with the implementation of
                      ;; let/eff (when used in let/eff*), or path
                      ;; isn't working properly in let/eff* ?
                      (append-value vl vr))]
    [nil/xt () (return/eff "")]
    [with/xt (x named body)
             (interp-with x named body)]
    [id/xt (x) (interp-id/xt x)]
    [aref/xt (x n) (interp-aref/xt x n)]))
...
(define (interp/xt xt0 ovr)
  (run/eff (interp/eff xt0) empty-env (list) ovr))
...
(test (interp/xt (with/xt 'x
                          ...
                          ;; this second y is not becoming x !!
                          (id/xt 'x))
...
(test (interp/xt
       (with/xt 'a (str/xt "hi")
                (with/xt 'b (id/xt 'a)
                         (with/xt 'c (id/xt 'a)
                                  (and/xt (id/xt 'b) (id/xt 'c)))))
       ;; the c doesn't get subbed with "jo" !!
       (list
...
(test (interp/xt
       (with/xt 'a (list (str/xt "way") (str/xt "free"))
                (and/xt (aref/xt 'a 1) (aref/xt 'a 0)))
       ;; the a[0] doesn't get subbed with a[1] !!
       (list
